...
A visible image on a LCD display is produced by both backlight and LCD
...
However, directly applying this technique to video playback faces
...
because two reasons.  On the one hand, the CPU may not have enough
time to perform these computation intensive tasks without degrading
the quality (e.g., frames per second (FPS)) of the video playback. On
the other hand, the achieved power savings by dimming backlight can be
offset by these extra CPU operations. Hsiu et al. and Lin et
al.~\cite{LHH14, HLH11} proposed to skip the pixels compensation stage
...
degrading due to backlight dimming. So far, no scheme has been
...
delay-sensitive. Furthermore, video conferencing often invovles
...
Facetime~\cite{skype, qq, facetime}, all support video
...
make the situation more complicated. To solve this fragmentation
...
IETF. Nowadays, mainstream browsers, e.g. Chrome, Firefox, Opera and
etc., all have integrated the WebRTC. The WebRTC
