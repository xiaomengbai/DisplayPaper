...
popularity among common mobile users. {\bf please add reference for
  each software}
...
display power saving scheme, called {\em LCD-GDP}.  Similar to the dynamic programming
approach for video streaming, there are same constraints that our
greedy algorithm must conform to. The {\em first} is that the backlight
variations between neighboring frames must be limited. Otherwise the flickering effect is expected
in the user experience~\cite{}. {\bf add reference here} The {\em second} is that the backlight should not be
...
To avoid processing the same frame repeatedly, in LCD-GDP, {\em first}, we migrate
...
{\em Second}, since GPU is commonly equipped in the mobile devices, we
...
results show that .....{\bf complete this paragraph}
